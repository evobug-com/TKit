...
@Riverpod(keepAlive: true)
TwitchAuthRemoteDataSource twitchAuthRemoteDataSource(Ref ref) {
...
@Riverpod(keepAlive: true)
CategoryMappingLocalDataSource categoryMappingLocalDataSource(Ref ref) {
  final database = ref.watch(appDatabaseProvider);
  return CategoryMappingLocalDataSource(database);
}

@Riverpod(keepAlive: true)
UnknownProcessDataSource unknownProcessDataSource(Ref ref) {
  final database = ref.watch(appDatabaseProvider);
...
MemoryCache memoryCache(Ref ref) {
  return MemoryCache(maxSize: 100, defaultTtl: const Duration(minutes: 30));
}

// Mapping list datasources
@Riverpod(keepAlive: true)
MappingListLocalDataSource mappingListLocalDataSource(Ref ref) {
  final database = ref.watch(appDatabaseProvider);
  return MappingListLocalDataSource(database);
}

@Riverpod(keepAlive: true)
MappingListSyncDataSource mappingListSyncDataSource(Ref ref) {
  final authDio = ref.watch(authDioProvider);
...
@Riverpod(keepAlive: true)
MappingSubmissionDataSource mappingSubmissionDataSource(Ref ref) {
  final authDio = ref.watch(authDioProvider);
  final logger = ref.watch(appLoggerProvider);
...
@Riverpod(keepAlive: true)
ProcessDetectionPlatformDataSource processDetectionPlatformDataSource(Ref ref) {
...
@Riverpod(keepAlive: true)
UpdateHistoryLocalDataSource updateHistoryLocalDataSource(Ref ref) {
  final database = ref.watch(appDatabaseProvider);
...
@Riverpod(keepAlive: true)
TwitchApiRemoteDataSource twitchApiRemoteDataSource(Ref ref) {
