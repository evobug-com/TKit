...
class UnknownProcessDataSource {
  ...
  Future<void> logUnknownProcess(
    ...
    try {
      ...
      final existing = await (database.select(database.unknownProcesses)
      ...
      if (existing != null) {
        ...
        await (database.update(database.unknownProcesses)
          ...
          UnknownProcessesCompanion(
            ...
            windowTitle: windowTitle != null ? Value(windowTitle) : const Value.absent(),
          ),
        );
      } else {
        // Create new entry
        await database.into(database.unknownProcesses).insert(
  ...
  Future<List<UnknownProcessModel>> getAllUnknownProcesses() async {
    try {
      final processes = await (database.select(database.unknownProcesses)
            ..orderBy([
              (tbl) => OrderingTerm(
