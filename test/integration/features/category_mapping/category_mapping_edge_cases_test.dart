...
void main() {
  ...
  group('Category Mapping Edge Cases', () {
    group('Concurrent Mapping Operations', () {
      test('should handle concurrent save operations for different mappings', () async {
        // arrange
        ...
        when(mockRepository.saveMapping(any))
      ...
      test('should handle race condition when saving same process twice', () async {
        // arrange
      ...
      test('should handle concurrent delete and find operations', () async {
        ...
        when(mockRepository.findMapping('game.exe', null))
            .thenAnswer((_) async {
          // Simulate delay
        ...
        when(mockRepository.deleteMapping(1))
            .thenAnswer((_) async {
          // Simulate delay
      ...
      test('should handle concurrent updateLastUsed calls', () async {
        ...
        when(mockRepository.updateLastUsed(mappingId))
            .thenAnswer((_) async {
          updateCount++;
    ...
    group('Special Characters and Unicode in Names', () {
      test('should handle category names with emojis', () async {
        ...
        when(mockRepository.saveMapping(mappingWithEmojis))
            .thenAnswer((_) async => const Right(null));
      ...
      test('should handle process names with special path characters', () async {
        // arrange
        final now = DateTime.now();
        ...
        for (final processName in specialProcessNames) {
          ...
          when(mockRepository.saveMapping(mapping))
              .thenAnswer((_) async => const Right(null));

          when(mockRepository.findMapping(processName, null))
              .thenAnswer((_) async => Right(mapping));

          // act
          final saveResult = await mockRepository.saveMapping(mapping);
          final findResult = await mockRepository.findMapping(processName, null);
      ...
      test('should handle very long category names (500+ characters)', () async {
        // arrange
        final now = DateTime.now();
        final longCategoryName = 'A' * 500 + ' - Very Long Category Name with lots of extra text to make it even longer and test the limits of our storage system';
        final mapping = CategoryMapping(
          id: 1,
          processName: 'game.exe',
    ...
    group('Cache Expiration Edge Cases', () {
      test('should handle mappings expiring at exact moment of check', () async {
        // arrange
        final now = DateTime.now();
        ...
        when(mockRepository.getMappingsExpiringSoon(const Duration(minutes: 1)))
            .thenAnswer((_) async => Right([aboutToExpire]));

        // act
        final result = await mockRepository.getMappingsExpiringSoon(const Duration(minutes: 1));

        // assert
        expect(result.isRight(), true);
        result.fold(
          (l) => fail('Should return expiring mappings'),
          (r) {
            expect(r.length, 1);
      ...
      test('should not delete manual override mappings even if expired', () async {
        // arrange
        final now = DateTime.now();
        ...
        when(mockRepository.getExpiredMappings())
            .thenAnswer((_) async => Right([expiredAuto])); // Only returns non-manual

        when(mockRepository.deleteExpiredMappings())
            .thenAnswer((_) async => const Right(1)); // Only deletes 1 (the auto)
        ...
        expiredResult.fold(
          (l) => fail('Should return expired mappings'),
          (r) {
            expect(r.length, 1);
    ...
    group('Fuzzy Matching Edge Cases', () {
      test('should find mapping with slight process name variations', () async {
        ...
        for (final variation in variations) {
          when(mockRepository.findMapping(variation, null))
              .thenAnswer((_) async => Right(mapping));
      ...
      test('should handle finding mapping with null path vs provided path', () async {
        // arrange
        final now = DateTime.now();
        final mapping = CategoryMapping(
          id: 1,
        ...
        when(mockRepository.findMapping('game.exe', 'C:\\Games\\MyGame\\game.exe'))
            .thenAnswer((_) async => Right(mapping));

        when(mockRepository.findMapping('game.exe', 'D:\\Different\\Path\\game.exe'))
            .thenAnswer((_) async => const Right(null)); // Different path might not match

        // act
        final resultNoPath = await mockRepository.findMapping('game.exe', null);
        final resultMatchingPath = await mockRepository.findMapping('game.exe', 'C:\\Games\\MyGame\\game.exe');
        final resultDifferentPath = await mockRepository.findMapping('game.exe', 'D:\\Different\\Path\\game.exe');
    ...
    group('Repository State Edge Cases', () {
      test('should handle deleting a mapping that does not exist', () async {
        // arrange
        when(mockRepository.deleteMapping(999))
      ...
      test('should handle empty repository operations', () async {
        // arrange
        when(mockRepository.getAllMappings())
      ...
      test('should handle disabled mappings in findMapping', () async {
        ...
        expect(result.isRight(), true);
        result.fold(
          (l) => fail('Should find disabled mapping'),
          (r) {
